...
def manipulador():
	for x in range(10):
		...
		for valor in list_configs:
			print valor
			p = subprocess.Popen('nvprof --metrics sm_efficiency ./gemm-cuda.exe 0 '+valor+' 512 512 512 0', shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
			...
			arquivo_csv = open("gemm-smefficiency-512-10tent-gtx-grid-2,1024,4,titanx-2tentativa.csv","a")
			arquivo_csv.write(str(x)+" tentativa\n")
			arquivo_csv.write("gx,gy,gz,bx,by,bz,smefficiency \n")
