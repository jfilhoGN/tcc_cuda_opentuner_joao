...
class gemmTuner(MeasurementInterface):
	...
	def run(self, desired_result, input, limit):
		...
		run_cmd = 'nvprof --metrics warp_execution_efficiency ./gemm-cuda.exe'
	...
	def get_metric_from_app_output(self, app_output, configuration, kernel):
		...
		for current_line in lines:
			strg = "" + current_line
			if strg.find("Warp Execution Efficiency") > -1:
				idx = strg.index("Warp Execution Efficiency")
				subsrtg = strg[idx:].split("    ")
				print "substrg: ", subsrtg
				substring = subsrtg[3]
				substring1 = substring.replace("%",'')
				metric_value = float(substring1)
				print "warp_execution_efficiency: ", metric_value
		...
		arquivo_csv = open("/home/projetocuda/Documentos/tcc_cuda_opentuner_joao/results/gtx780/gemm-warpefficiency-"+str(sys.argv[2])+".csv","a")
		arquivo_csv.write("Kernel,gx,gy,gz,bx,by,bz,gpuId,efficiency-warp-execution \n")
