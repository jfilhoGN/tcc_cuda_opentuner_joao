...
class gemmTuner(MeasurementInterface):
	...
	def run(self, desired_result, input, limit):
		...
		run_cmd = 'nvprof --metrics gst_efficiency ./gemm-cuda.exe'
	...
	def get_metric_from_app_output(self, app_output, configuration, kernel):
		...
		for current_line in lines:
			strg = "" + current_line
			if strg.find("Global Memory Store Efficiency") > -1:
				idx = strg.index("Global Memory Store Efficiency")
				subsrtg = strg[idx:].split("    ")
				print "substrg: ", subsrtg
				substring = subsrtg[3]
				substring1 = substring.replace("%",'')
				metric_value = float(substring1)
				print "gst_efficiency: ", metric_value
		...
		arquivo_csv = open("/home/joao/Documentos/tcc_cuda_opentuner_joao/results/gtx1070/gemm-gst_efficiency-"+str(sys.argv[2])+".csv","a")
