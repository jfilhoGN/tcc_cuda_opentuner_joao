...
# --------------------------------------------------------------------
  def run(self, desired_result, input, limit):
    ...
    if not compiled:
      ...
      # MUDAR AQUI-----------
    run_cmd = 'nvprof --metrics inst_per_warp ./sincosc-cuda'
  ...
  def get_metric_from_app_output(self, app_output, configuration, kernel):
    ...
    for current_line in lines:
      ...
      if strg.find("Instructions per warp") > -1:
        ...
        print "inst_per_warp: ", metric_value
    ...
    # MUDAR AQUI-----------
    arquivo_csv = open("/home/joao/Documentos/tcc_cuda_opentuner_joao/results/gtx1070/sincos-inst_per_warp-"+str(sys.argv[2])+".csv","a")
...
# --------------------------------------------------------------------
  def save_final_config(self, configuration):
    ...
    # MUDAR AQUI-----------
    self.manipulator().save_to_file(configuration.data, 'sincoscuda_final_config_inst_per_warp'+str(sys.argv[2])+'.json')
