...
# --------------------------------------------------------------------
  def run(self, desired_result, input, limit):
    ...
    run_cmd = 'nvprof --metrics shared_efficiency ./sincosc-cuda'
...
# --------------------------------------------------------------------
  def get_metric_from_app_output(self, app_output, configuration, kernel,gpuId):
    ...
    for current_line in lines:
      strg = "" + current_line
      if strg.find("Shared Memory Efficiency") > -1:
        idx = strg.index("Shared Memory Efficiency")
        subsrtg = strg[idx:].split("    ")
        print "substrg: ", subsrtg
        substring = subsrtg[3]
        substring1 = substring.replace("%",'')
        metric_value = float(substring1)
        print "shared_efficiency: ", metric_value
    ...
    arquivo_csv = open("/home/joao/Documentos/tcc_cuda_opentuner_joao/results/gtx1070/sincos-shared_efficiency-executed-"+str(sys.argv[2])+".csv","a")
...
# --------------------------------------------------------------------
  def save_final_config(self, configuration):
    ...
    self.manipulator().save_to_file(configuration.data, 'sincoscuda_final_config_'+str(sys.argv[2])+'.json')
